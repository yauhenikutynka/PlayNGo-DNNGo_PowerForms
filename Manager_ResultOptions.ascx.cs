...
{
    public partial class Manager_ResultOptions : basePortalModule
    {
        ...
        {
            EffectDB XmlDB = Setting_ResultDB;
            if (!(XmlDB != null && String.IsNullOrEmpty(XmlDB.Name)))
            {
                ...
                //绑定效果的主题
                String EffectDirPath = String.Format("{0}Results/{1}/Themes/", Server.MapPath(ModulePath), XmlDB.Name);
                ...
                if (ThemeDirs != null && ThemeDirs.Length > 0)
                {
                    foreach (DirectoryInfo dir in ThemeDirs)
                    {
                        ...
                        FileInfo imgFile = new FileInfo(MapPath(String.Format("{0}Results/{1}/Themes/{2}/image.jpg", ModulePath, XmlDB.Name, dir.Name)));
                        if (imgFile.Exists)
                            dirEntity.Value = String.Format("{0}Results/{1}/Themes/{2}/image.jpg", ModulePath, XmlDB.Name, dir.Name);
                        else
                            dirEntity.Value = String.Format("http://www.dnngo.net/DesktopModules/DNNGo_PowerForms/Results/{0}/Themes/{1}/image.jpg", XmlDB.Name, dir.Name);
                ...
                WebHelper.SelectedListByText(ddlThemeName, Settings_ResultThemeName);


                // hfThemeThumbnails.Value = String.Format("{0}Effects/{1}/Themes/[ResultThemeName]/image.jpg", ModulePath, XmlDB.Name);
                if (!String.IsNullOrEmpty(Settings_ResultThemeName))
                {
                    ...
                    imgThemeThumbnails.ToolTip = Settings_ResultThemeName;
                    KeyValueEntity dirEntity = dirs.Find(r1 => r1.Key.IndexOf(Settings_ResultThemeName, StringComparison.CurrentCultureIgnoreCase) >= 0);
        ...
        private void SetDataItem()
        {
            ...
            UpdateModuleSetting("PowerForms_ResultThemeName", ddlThemeName.Items[ddlThemeName.SelectedIndex].Text);
        ...
        private void SetThemeSettings()
        {
            ...
            //获取效果参数
            List<SettingEntity> EffectSettingDB = Setting_ResultSettingDB;
        ...
        protected void cmdUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Response.Redirect(xUrl("ResultOptions"), false);
            }
            catch (Exception ex)
            {
                ProcessModuleLoadException(ex);
        ...
        {
            //获取效果参数
            List<GroupEntity> ItemSettingDB = Setting_ResultGroupsDB;

            if (!(ItemSettingDB != null && ItemSettingDB.Count > 0))
            {
                //当分组都没配置的时候，需要取出参数里面的分组

                foreach (var ItemSetting in Setting_ResultSettingDB)
                {
            ...
            ////获取效果参数
            //List<SettingEntity> ItemSettingDB = Setting_ResultSettingDB;
