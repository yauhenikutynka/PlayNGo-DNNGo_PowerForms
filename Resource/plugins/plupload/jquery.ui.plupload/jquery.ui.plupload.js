		...
		'<div class="plupload_wrapper">' +
			'<div class="plupload_container">' +
				...
				'<tr>' +
					...
					'</td>' +
					'<td class="plupload_started plupload_hidden"><!-- Hidden -->' +
...
$.widget("ui.plupload", {
	...
	options: {
		browse_button_hover: 'state-hover',
		browse_button_active: 'state-active',
	...
	_create: function() {
		...
		if ($.ui.button) {
			this.browse_button.button({
				icons: { primary: 'icon-circle-plus' },
				disabled: true
			});
			
			this.start_button.button({
				icons: { primary: 'icon-circle-arrow-e' },
				disabled: true
			});
			
			this.stop_button.button({
				icons: { primary: 'icon-circle-close' }
			});
      
			this.list_switcher.button({
				text: false,
				icons: { secondary: "icon-grip-dotted-horizontal" }
			});

			this.thumbs_switcher.button({
				text: false,
				icons: { secondary: "icon-image" }
			});
		}
	...
	notify: function(type, message) {
		var popup = $(
			'<div class="plupload_message">' + 
				'<span class="plupload_message_close icon icon-circle-close" title="'+_('Close')+'"></span>' +
				'<p><span class="icon"></span>' + message + '</p>' +
			'</div>'
		);
					
		popup
			.addClass('state-' + (type === 'error' ? 'error' : 'highlight'))
			.find('p .icon')
				.addClass('icon-' + (type === 'error' ? 'alert' : 'info'))
				.end()
	...
	*/
	...
	_handleState: function() {
		...
		if (up.total.queued === 0) {
			$('.button-text', this.browse_button).html(_('Add Files'));
		} else {
			$('.button-text', this.browse_button).html(o.sprintf(_('%d files queued'), up.total.queued));
		}
	...
	_handleFileStatus: function(file) {
		...
		switch (file.status) {
			case plupload.DONE: 
				...
				iconClass = 'plupload_action_icon icon icon-circle-check';
				break;
			
			case plupload.FAILED:
				actionClass = 'state-error plupload_failed';
				iconClass = 'plupload_action_icon icon icon-alert';
				break;

			case plupload.QUEUED:
				actionClass = 'plupload_delete';
				iconClass = 'plupload_action_icon icon icon-circle-minus';
				break;

			case plupload.UPLOADING:
				actionClass = 'state-highlight plupload_uploading';
				iconClass = 'plupload_action_icon icon icon-circle-arrow-w';
		...
		actionClass += ' state-default plupload_file';
	...
	_addFiles: function(files) {
		...
		file_html = '<li class="plupload_file state-default plupload_delete" id="{id}" style="width:{thumb_width}px;">' +
			'<div class="plupload_file_name" title="{name}">' +
				'<span class="plupload_file_name_wrapper">{name} </span>' +
			'</div>' +						
			'<div class="plupload_file_fields"> </div>' +
			'<div class="plupload_file_status">' +
				'<div class="plupload_file_progress widget-header" style="width: 0%"> </div>' + 
				'<span class="plupload_file_percent">{percent} </span>' +
			'</div>' +
			'<div class="plupload_file_size">{size} </div>' +
			'<div class="plupload_file_action">' +
				'<div class="plupload_action_icon icon icon-circle-minus"> </div>' +
			'</div>' +
		'</li>';
