...
{
    public partial class Manager_ResultStyle : basePortalModule
    {
        ...
        {
            EffectDB ResultDB = Setting_ResultDB;
            if (!(ResultDB != null && String.IsNullOrEmpty(ResultDB.Name)))
            {
 

                //绑定样式表到内容框
                String StyleFile = Server.MapPath(String.Format("{0}Results/{1}/Themes/{2}/Style.css", ModulePath, Settings_ResultName, Settings_ResultThemeName));
        ...
        protected void cmdUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Response.Redirect(xUrl("ResultStyle"), true);
            }
            catch (Exception ex)
            {
                DotNetNuke.Services.Exceptions.Exceptions.ProcessModuleLoadException(this, ex);
        ...
        protected void cmdCancel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Response.Redirect(xUrl("ResultOptions"), true);
            }
            catch (Exception ex)
            {
                DotNetNuke.Services.Exceptions.Exceptions.ProcessModuleLoadException(this, ex);
